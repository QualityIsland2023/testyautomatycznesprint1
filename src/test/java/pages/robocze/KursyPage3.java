...
public class KursyPage3 {
    ...
    public KursyPage3(WebDriver driver) {
        this.driver = driver;
    ...
    public boolean sprawdzCzyButtonUtworzNowyKursIstnieje() {
        ...
        if (wait.waitForVisibility(utworzNowyKursButton).isDisplayed() && wait.waitForVisibility(utworzNowyKursButton).isEnabled()) {
            status = true;
    ...
    public boolean sprawdzCzyButtonTypyDanychIstnieje() {
        boolean status = false;
        if (wait.waitForVisibility(typyDanychButton).isDisplayed() && wait.waitForVisibility(typyDanychButton).isEnabled()) {
            status = true;
    ...
    public Boolean sprawdzCzyPoleNazwaKursuIstnieje() {
        boolean status = false;
        if (wait.waitForVisibility(nazwaKursuInput).isDisplayed() && wait.waitForVisibility(nazwaKursuInput).isEnabled()) {
            status = true;
    ...
    public Boolean sprawdzCzyPoleCenaIstnieje() {
        ...
        if (wait.waitForVisibility(cenaKursuInput).isDisplayed() && wait.waitForVisibility(cenaKursuInput).isEnabled()) {
            status = true;
            System.out.println("Pole 'Cena' jest wyświetlone i aktywne.");
        } else {
            System.out.println("Pole 'Cena' NIE jest wyświetlone i/lub aktywne.");
        }
        return status;
    }
    ...
    public Boolean sprawdzCzyPoleNazwaKursuICenaIstnieje() {
        ...
        if (wait.waitForVisibility(nazwaKursuInput).isDisplayed() &&
            ...
            System.out.println("Pola 'Nazwa kursu' i 'Cena' nie są wyświetlone i/lub aktywne.");
        }
        return status;
    }

    //metoda wypisukąca, czy przycisk 'Utwórz i edytuj' jest widoczny i aktywny
    public void sprawdzPrzyciskUtworzIEdytuj() {
        if (utworzIEdytujButton.isDisplayed()) {
            System.out.println("Przycisk 'Utwórz i edytuj' jest wyświetlony.");
        } else {
            System.out.println("Przycisk 'Utwórz i edytuj' NIE jest WYŚWIETLONY.");
        }
        if (utworzIEdytujButton.isEnabled()) {
            System.out.println("Przycisk 'Utwórz i edytuj' jest aktywny.");
        } else {
    ...
    public Boolean sprawdzCzyPrzyciskUtworzIEdytujIstnieje () {
        ...
        if (wait.waitForVisibility(utworzIEdytujButton).isDisplayed() &&
            ...
            System.out.println("Przycisk 'Utwórz i edytuj'  nie jest wyświetlony i/lub aktywny.");
        }
        return status;
    }
